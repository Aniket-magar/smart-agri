...
if (isset($_GET['city'])) {
    ...
    $apiKey = "56b5d83e8d808249988fc63e9b4470cb";
    $url = "http://api.openweathermap.org/data/2.5/forecast?q=" . urlencode($city) . "&appid={$apiKey}&units=metric";
        ...
        if (isset($data['list'])) {
            ...
            foreach ($data['list'] as $entry) {
                $forecast[] = [
                    ...
                    "weather" => ucfirst($entry['weather'][0]['description']),
...
<div class="nav-list">
    ...
    <a href="weather-forecasting.php">Weather</a>
...
<body>
    <div class="container mt-5 text-center">
        ...
        <input type="text" id="city" class="form-control w-50 d-inline" placeholder="Enter city name">
        <button class="btn btn-warning mt-2" onclick="fetchWeather()">Get Weather</button>
            ...
            fetch("?city=" + encodeURIComponent(city))
                ...
                .then(data => {
                    ...
                    if (data.error) {
                        ...
                        let forecastHTML = "<div class='d-flex overflow-auto gap-3 p-2'>";
                        let labels = [];
                        let temps = [];
                        data.forEach(entry => {
                            forecastHTML += `<div class='border p-3 text-center'>
                                                <p>${entry.time}</p>
                                                <img src='${entry.icon}' alt='${entry.weather}'>
                ...
                .catch(() => document.getElementById("result").innerHTML = "<p class='text-danger'>Error fetching data.</p>");
        }
    </script>
